...
test('user_helpers dev', function(done, fail) {
    var stored = {};
    mock(
        'user_helpers',
        {
            storage: {getItem: function(key) { return stored[key]; }},
            user: {}
        },
        function(user_helpers) {
            eq_(user_helpers.dev('android'), 'android');
            eq_(user_helpers.dev('firefoxos'), 'firefoxos');
            eq_(user_helpers.dev('desktop'), 'desktop');

            stored.device_filtering = true;
            eq_(user_helpers.dev('android'), 'android');
            eq_(user_helpers.dev('firefoxos'), 'firefoxos');
            eq_(user_helpers.dev('desktop'), 'desktop');

            stored.device_filtering = false;
            eq_(user_helpers.dev('android'), 'android');
            eq_(user_helpers.dev('firefoxos'), 'firefoxos');
            eq_(user_helpers.dev('desktop'), null);
