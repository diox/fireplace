...
function(_) {
    ...
    var thumbnailsDeferrer = ImageDeferrer.Deferrer(null, 200);
    z.page.one('loaded', function() {
        iconDeferrer.setImages($('.icon.deferred'));
        thumbnailsDeferrer.setImages($('.screenshot img.deferred'));
    }).on('loaded loaded_more navigate fragment_loaded transitionend webkitTransitionEnd', function() {
        iconDeferrer.refresh();
        thumbnailsDeferrer.refresh();
    });
    require('nunjucks').require('globals').imgAlreadyDeferred = function(src) {
        ...
        */
        ...
        var thumbnailsLoaded = thumbnailsDeferrer.getSrcsAlreadyLoaded();
